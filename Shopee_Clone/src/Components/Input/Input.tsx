...
export default function Input({
  ...
  const match = useMatch(`${path.home}/user/password`)
  ...
  const renderEyes = () => {
    return eyes ? (
      <svg
        ...
        strokeWidth={1.5}
        stroke='currentColor'
        className='size-6 absolute right-0 -translate-y-[30px] mr-2'
      >
        <path
          strokeLinecap='round'
  ...
  return (
    <div className={className}>
      {!match && <input className={classNameInput} type={type} placeholder={placeholder} {...registerResult} />}
      {match && (
        <div>
          <input
